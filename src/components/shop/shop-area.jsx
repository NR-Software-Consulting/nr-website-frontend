...
const ShopArea = ({
  ...
  const handleCategoryFilter = async (categoryId, category, _selectedCategories) => {
    try {
      let updatedCategories = _selectedCategories ? _selectedCategories : [...selectedCategories];
      let selectedSubs = []
      if (updatedCategories.includes(categoryId)) {
        ...
        categories
          ...
          .attributes.sub_categories.data.map(subCategory => {
            selectedSubs = selectedSubs.filter((item) => item !== subCategory?.id)
          });
      } else {
        ...
        selectedSubs = _selectedCategories ? selectedSubs : [...selectedSubCategories, ...selectedSubs]
      }
      ...
      if (priceValue[0] > 0 || priceValue[1] < maxPrice) {
        newData.variables.filters.price = {
          between: [priceValue[0] == 0 ? priceValue[0] : priceValue[0] - 1, priceValue[1] + 1],
        }
      }
      ...
      if (response.data.products.data) {
        ...
        const slugs = cateoryfilteredProducts.map(item => item?.attributes?.slug).filter(Boolean);
  ...
  // subCategory Filter
  const handleSubCategoryFilter = async (subCategoryId, categoryId, category, subCategory, _selectedSubCategories, categoryItem) => {
    try {

      let updatedSubCategories = _selectedSubCategories ? _selectedSubCategories : [...selectedSubCategories];
      if (updatedSubCategories && updatedSubCategories.includes(subCategoryId)) {
        updatedSubCategories = updatedSubCategories.filter(id => id !== subCategoryId);
      } else {
        updatedSubCategories.push(subCategoryId);
      }

      const _selectedCategories = categoryItem?.id == undefined ? [] : [...selectedCategories]
      if (categoryItem?.attributes?.sub_categories?.data?.filter((item) => updatedSubCategories.includes(item?.id))?.length == 0) {
        setSelectedCategories(_selectedCategories.filter((item) => item !== categoryId))
      } else {
      ...
      if (response.data.products.data) {
        ...
        const slugs = subCateoryfilteredProducts.map(item => item?.attributes?.slug).filter(Boolean);
        const newUrl = `${router.pathname}?${slugs.join("&")}`;
        window.history.replaceState({}, "", newUrl);
  ...
  const handleSortingFilter = async (selectValue) => {
    try {
      ...
      if (response.data.products.data) {
        ...
        const slugs = SortingFilteredProducts
          .map((item) => item?.attributes?.slug)
          .filter(Boolean);
        const newUrl = `${router.pathname}?${slugs.join("&")}`;
        window.history.replaceState({}, "", newUrl);
